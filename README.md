...
- 分享 token 带有基于 `COOKIE_SECRET` 的 HMAC，并签入 proof 哈希；普通 GET、聊天软件链接预览、篡改 token、错误 proof 和非 JSON 请求都会在 D1 查询前被拒绝。
...
- 轮换 `COOKIE_SECRET` 会使尚未领取的分享链接立即失效；对应密文行会在过期清理时删除。
